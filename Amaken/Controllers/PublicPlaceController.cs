...
public class Public_PlaceController : Controller
{
    ...
    public IActionResult UpdatePublicPlace([FromBody] Public_Place updatedPlace)
    {
        ...
        {
            var PublicPlace =
                _context.Public_Place.FirstOrDefault(u => u.PublicPlaceId!.Equals(updatedPlace.PublicPlaceId));
            if (PublicPlace != null)
            {
    ...
    public IActionResult SearchPublicPlaces()
    {
        var publicPlacesWithRates = _context.Public_Place
            .Select(place => new
            {
                Place = place,
                NumberOfRates =
                    _context.PlacesRates.Count(r => r.PlaceId.ToLower().Equals(place.PublicPlaceId.ToLower())),
                AverageScore = _context.PlacesRates
                    .Where(r => r.PlaceId.ToLower().Equals(place.PublicPlaceId.ToLower()))
    ...
    public async Task<IActionResult> GetPlace(string id)
    {
        ...
        if (MyUser == null)
        {
            var place = await _context.Public_Place
                ...
                .Select(
                    place => new
                    {
                        DidUserRate = false,
                        Place = place,
                        NumberOfRates = _context.PlacesRates.Count(r =>
                            r.PlaceId.ToLower().Equals(place.PublicPlaceId.ToLower())),
                        AverageScore = _context.PlacesRates
                            .Where(r => r.PlaceId.ToLower().Equals(place.PublicPlaceId.ToLower()))
                            .Average(r => (double?)r.Score) ?? 0
                    }
            ...
            if (place == null)
                throw new Exception($"Place with id {id} was not found");

            return Ok(place);
        }
        else
        {
            bool MyDidUserRate = await _context.PlacesRates
                .AnyAsync(r => r.UserEmail.ToLower() == MyUser.Email.ToLower() && r.PlaceId.ToLower() == id.ToLower());
            var place = await _context.Public_Place
                .Where(e => e.PublicPlaceId == id)
                .Select(
                    place => new
                    {
                        DidUserRate = MyDidUserRate,
                        Place = place,
                        NumberOfRates = _context.PlacesRates.Count(r =>
                            r.PlaceId.ToLower().Equals(place.PublicPlaceId.ToLower())),
                        AverageScore = _context.PlacesRates
    ...
    public IActionResult SavePublicPlace(string placeId)
    {
        ...
        if (MyUser != null)
        {
            ...
            if (myPlace != null)
            {
                var list = new List<string>(MyUser.SavedPublicPlaces ?? new string[0]);
    ...
    public async Task<IActionResult> GetCity(double latitude, double longitude)
    {
        ...
        if (city != null) return Ok(new { City = city });
    ...
    {
        var isUnique = !_context.Public_Place.Any(p => p.Name.ToLower() == name.ToLower());
        return Ok(isUnique);
    }
}